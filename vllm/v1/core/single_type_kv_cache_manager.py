...
class OscarMLAKVCacheManager(FullAttentionManager):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.enable_caching:
            raise ValueError("OSCAR MLA does not support prefix caching")
    ...
    def get_num_blocks_to_allocate(
        ...
        if new_computed_blocks:
            raise ValueError("OSCAR MLA does not support external or prefix cache hits")
        if num_tokens != num_tokens_main_model:
            raise ValueError("OSCAR MLA does not support speculative lookahead")
        if not self.tri_pool.can_update_length(request_id, num_tokens):
    ...
    def allocate_new_computed_blocks(
        ...
        if new_computed_blocks or num_external_computed_tokens:
            raise ValueError("OSCAR MLA does not support external or prefix cache hits")
    ...
    def allocate_new_blocks(
        ...
        try:
            ...
            self.tri_pool.update_length(request_id, num_tokens)
        except Exception:
            if new_blocks:
                req_blocks = self.req_to_blocks[request_id]
            ...
            if is_new_request and request_id in self.tri_pool.requests:
